...
public class QueueController {
    ...
    //Remove um agendamento da fila e reorganiza as posições.
    @DeleteMapping("/{id}")
    public ResponseEntity<?> removeFromQueue(@PathVariable Long id) {
        try {
            queueService.removeFromQueue(id);
            return ResponseEntity.ok("Agendamento removido da fila com sucesso");
        } catch (RuntimeException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body("Erro: " + e.getMessage());
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Erro ao remover da fila: " + e.getMessage());
        }
    }
